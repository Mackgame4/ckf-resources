--[[
						NativeUI
	Dont edit above, unless you know what are you doing
]]--
...
local function debugPrint(text)
    if NativeUI.debug then
        Citizen.Trace('[NativeUI] '..tostring(text))
    end
end
...
-- API
function NativeUI.CreateMenu(id, title, closeCallback)
    -- Default settings
    menus[id] = { }
...
function NativeUI.CreateSubMenu(id, parent, subTitle, closeCallback)
    if menus[parent] then
        NativeUI.CreateMenu(id, menus[parent].title)
...
function NativeUI.CurrentMenu()
    return currentMenu
end

function NativeUI.MenuTable()
    return menus
end
...
function NativeUI.IsMenuOpened(id)
    return isMenuVisible(id)
end

function NativeUI.IsAnyMenuOpened()
...
function NativeUI.IsAnyMenuWithTitleOpened(subtitle)
...
function NativeUI.IsMenuAboutToBeClosed()
    if menus[currentMenu] then
...
function NativeUI.IsThisMenuAboutToBeClosed(id)
    if menus[id] then
...
function NativeUI.Button(text, subText)
...
function NativeUI.DisabledButton(text, subText)
    local buttonText = text
    if subText then
        buttonText = '{ '..tostring(buttonText)..', '..tostring(subText)..' }'
    end
...
function NativeUI.SpriteButton(text, textDict, sprite, focusSprite)
    local buttonText = text
    if menus[currentMenu] then
        optionCount = optionCount + 1
        local isCurrent = menus[currentMenu].currentOption == optionCount
...
function NativeUI.SpriteMenuButton(text, textDict, sprite, focusSprite, id)
    if menus[id] then
        local clicked, hovered = NativeUI.SpriteButton(text, textDict, sprite, focusSprite)
        if clicked then
            setMenuVisible(currentMenu, false)
...
function NativeUI.MenuButton(text, id, secondtext)
    if menus[id] then
        if NativeUI.Button(text, (secondtext and secondtext or "→")) then
            setMenuVisible(currentMenu, false)
            setMenuVisible(id, true, true)
            return true
        end
...
function NativeUI.SwitchMenu(id)
	setMenuVisible(currentMenu, false)
    setMenuVisible(id, true, true)
end

function NativeUI.CheckBox(text, bool, callback)
    --[[local checked = '~r~Off'
    if bool then
        checked = '~g~On'
    end]]--
    ...
    if NativeUI.SpriteButton(text, "commonmenu", sprite, focusSprite) then
        bool = not bool
...
function NativeUI.ComboBox(text, items, currentIndex, selectedIndex, callback, displaycb)
    ...
    if NativeUI.Button(text, selectedItem) then
...
function NativeUI.SetMenuWidth(id, width)
    setMenuProperty(id, 'width', width)
end

function NativeUI.SetMenuX(id, x)
    setMenuProperty(id, 'x', x)
end

function NativeUI.SetMenuY(id, y)
    setMenuProperty(id, 'y', y)
end

function NativeUI.SetMenuMaxOptionCountOnScreen(id, count)
    setMenuProperty(id, 'maxOptionCount', count)
end

function NativeUI.SetTitleColor(id, r, g, b, a)
    setMenuProperty(id, 'titleColor', { r, g, b, a or  menus[id].titleColor[4] })
end

function NativeUI.SetTitleBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'titleBackgroundColor', { r, g, b, a or menus[id].titleBackgroundColor[4] })
end

function NativeUI.UseSpriteAsBackground(id, textDict, sprite, r, g, b, a, stillDrawText)
